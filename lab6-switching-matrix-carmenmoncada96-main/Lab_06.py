...
class Network:
    ...
    def connect(self):
        ...
        for node in self.nodes:
            ...
            for line in self.lines:   # ['AB']
                if line.startswith(node):
                    #node_successive_list.append(self.lines[line].successive)
            ...
            for node_destination in self.nodes:  # node destination
                if node_source != node_destination:  # Finding all possible paths starting from A-B
                    ...
                    for index1 in range(len(paths_list)):  # Iterate over each list
                        ...
                        signal_info = self.probe(signal_info)                 # probe method = old propagate method
    ...
    def find_best_latency(self, node1, node2):
    ...
    def route_space_update(self,best_path,best_path_index):

        channel = np.ones(NUMBER_OF_CHANNELS, dtype=int)
        path_str = str(best_path).split('->')
        ...
        self.route_space.iloc[best_path_index, 1:] = channel
    ...
    def stream(self, connection_list, signal_power, label):
        ...
        for list in range(len(connection_list)):
            if (label == 'latency') or (label == 'Latency'):
                best_path_index, index_channel = self.find_best_latency(connection_list[list].input,
                                                                        connection_list[list].output)
                print(best_path_index)
            else:  # if (label == 'snr') or (label == 'SNR'):
                # For the SNR
                best_path_index, index_channel = self.find_best_snr(connection_list[list].input,
                                                                    connection_list[list].output)
            ...
            if best_path_index != -1:
                ...
                if best_path_index != -1 and index_channel != -1:
                    self.route_space_update(best_path, best_path_index)
